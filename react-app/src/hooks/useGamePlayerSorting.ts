...
export interface UseGamePlayerSortingOptions {
  currentQueue?: string[]
  fallbackSort?: 'queue_position' | 'name'
}

export function useGamePlayerSorting(
  players: GamePlayer[],
  options: UseGamePlayerSortingOptions = {}
) {
  const {
    currentQueue,
  ...
  const sortedPlayers = useMemo(() => {
    ...
    if (currentQueue && currentQueue.length > 0) {
      ...
      console.log(`🎮 useGamePlayerSorting: Игроки отсортированы по current_queue:`, 
    ...
    console.log(`🎮 useGamePlayerSorting: Используем fallback сортировку по ${fallbackSort}`)
  ...
  }, [players, currentQueue, fallbackSort])
