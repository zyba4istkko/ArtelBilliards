#!/usr/bin/env python3
"""
Migration initialization script for Game Service
...
async def wait_for_database():
    ...
    db_config = {
        ...
        'dbname': os.getenv('DB_NAME', 'game_db'),  # psycopg3 использует 'dbname' вместо 'database'
...
def init_alembic():
    ...
    if not Path("alembic.ini").exists():
        ...
        db_url = f"postgresql+psycopg://{os.getenv('DB_USER', 'postgres')}:{os.getenv('DB_PASSWORD', 'password')}@{os.getenv('DB_HOST', 'postgres')}:{os.getenv('DB_PORT', '5432')}/{os.getenv('DB_NAME', 'game_db')}"
...
def update_env_py():
    ...
    # Create new env.py
    env_py_content = '''"""
Alembic environment configuration for Game Service
"""
...
def get_url():
    ...
    return f"postgresql+psycopg://{os.getenv('DB_USER', 'postgres')}:{os.getenv('DB_PASSWORD', 'password')}@{os.getenv('DB_HOST', 'postgres')}:{os.getenv('DB_PORT', '5432')}/{os.getenv('DB_NAME', 'game_db')}"
...
def create_initial_migration():
    ...
    if not existing_migrations:
        ...
        if not run_command("alembic revision --autogenerate -m 'Initial tables for Game Service'"):
...
async def main():
    ...
    print("🚀 Starting Game Service migration process...")
